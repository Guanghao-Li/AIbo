...
from aibo.core.skills.loader import SkillLoader
from aibo.core.transport.socket_client import IpcError, SocketClient
...
class AiboTuiApp(App[None]):
    """AIBO TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]AIBO[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]AIBO[/bold]  [dim]{self._host}:{self._port}[/dim]"
    ...
    async def _socket_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                header.update(f"[bold]AIBO[/bold]  [red]subscribe error: {e}[/red]")
            finally:
...
# TUI 入口：读取配置并启动 AiboTuiApp
def run(config: AiboConfig, replay_run_id: str | None = None) -> None:
    app = AiboTuiApp(config.host, config.port, replay_run_id=replay_run_id)
    app.run()
