...
from aibo.core.bus.events import RunFinishedEvent, RunStartedEvent
from aibo.core.compact.compactor import Compactor
from aibo.core.config import AiboConfig
from aibo.core.context import ExecutionContext
from aibo.core.events.bus import EventBus, EventHandler
from aibo.core.events.writer import EventWriter
from aibo.core.llm.base import LLMProvider
from aibo.core.llm.provider import AnthropicProvider
from aibo.core.loop import AgentLoop
from aibo.core.mcp.server import McpServerManager
from aibo.core.memory.loader import load_context_file
from aibo.core.permissions.manager import PermissionManager
from aibo.core.runs import RUNS_DIR, new_run_id
from aibo.core.session.model import Session
from aibo.core.session.store import SessionStore
from aibo.core.subagent.registry import BackgroundTaskRegistry
from aibo.core.subagent.tool import AgentResultTool, SpawnAgentTool
from aibo.core.task.manager import TaskManager
from aibo.core.tools.builtin import (
    BashTool,
    ListDirTool,
    NoteSaveTool,
    ReadFileTool,
...
from aibo.core.trace.provider import TracingProvider
...
class AgentRunner:
    ...
    async def run_and_capture(
        ...
        *,
        ...
        global_ctx = load_context_file(Path("~/.aibo/context.md").expanduser())
        project_ctx = load_context_file(Path(".aibo/context.md"))
        ...
        async with EventWriter(run_path / "events.jsonl") as writer:
            ...
            try:
                ...
                logging.getLogger(__name__).exception(
                    "agent provider initialization failed run_id=%s", run_id
                )
                if not context.is_done():
