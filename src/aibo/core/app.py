...
import aibo
from aibo.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from aibo.core.bus.envelope import EventPushEnvelope
from aibo.core.config import AiboConfig, get_config
from aibo.core.events.bus import EventBus
from aibo.core.llm.provider import AnthropicProvider
from aibo.core.logging_setup import setup_logging
from aibo.core.mcp.server import McpServerManager
from aibo.core.permissions.manager import PermissionManager
from aibo.core.permissions.storage import load_policy_file
from aibo.core.runner import AgentRunner
from aibo.core.runs import events_file, new_run_id
from aibo.core.session import SessionManager, SessionStore
from aibo.core.trace.record import TraceRecord
from aibo.core.trace.writer import TraceWriter
from aibo.core.transport.ipc_broadcaster import IpcEventBroadcaster
from aibo.core.transport.socket_server import SocketServer, get_connection_writer
...
def _install_shutdown_handlers(
    ...
    try:
        ...
        def restore_loop_handlers() -> None:
            for sig in signals:
                loop.remove_signal_handler(sig)

        return restore_loop_handlers
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: AiboConfig | None = None
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=aibo.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.aibo/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.aibo/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.aibo/sessions").expanduser()
        ...
        self._sessions = SessionManager(
            ...
            bus=self._bus,
            provider_factory=lambda: AnthropicProvider(self._config.llm.default_model),  # type: ignore[union-attr]
        )
        ...
        logger.info("aibo-core %s listening addr=%s", aibo.__version__, addr)
        logger.info("config: %s", self._config)

        loop = asyncio.get_running_loop()
        shutdown = asyncio.Event()
        restore_shutdown_handlers = _install_shutdown_handlers(loop, shutdown)
