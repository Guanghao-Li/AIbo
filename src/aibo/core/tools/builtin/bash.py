...
_DEFAULT_TIMEOUT = 60
_SLEEP_RE = re.compile(r"^\s*sleep\s+([0-9]+(?:\.[0-9]+)?)\s*$")
...
class BashTool(BaseTool):
    ...
    async def invoke(self, params: dict[str, object]) -> ToolResult:
        ...
        try:
            sleep_match = _SLEEP_RE.match(command)
            if sleep_match is not None:
                try:
                    await asyncio.wait_for(asyncio.sleep(float(sleep_match.group(1))), timeout=timeout)
                except TimeoutError:
                    return ToolResult(
