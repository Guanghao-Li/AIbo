...
import aibo
from aibo.core.bus.commands import PongResult
from aibo.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from aibo.core.config import AiboConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: AiboConfig) -> None:
    try:
...
async def _ping(config: AiboConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{aibo.__version__}"},
    }
