...
# Run daemon manually
uv run aibo-core                        # foreground; Ctrl+C to stop
AIBO_PORT=8000 uv run aibo-core        # override port
...
This is a **dual-process** local AI agent system. `aibo-core` is a persistent daemon; `aibo` and `aibo-tui` are clients that connect to it over a Unix domain socket.

```
aibo-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
aibo (CLI)   aibo-tui (TUI, S2+)
```

**`aibo-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `aibo` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/aibo/core/bus/`)
...
### Transport layer (`src/aibo/core/transport/`)
...
### Config (`src/aibo/core/config.py`)

Four-tier priority: **built-in defaults → `~/.aibo/config.toml` → `.env` → env vars**.
...
Relevant env vars: `AIBO_CONFIG`, `AIBO_HOST`, `AIBO_PORT`, `AIBO_LOG_LEVEL`, `AIBO_LOG_FILE`, `AIBO_LOG_FORMAT`.

### Daemon entry (`src/aibo/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `AIBO_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
